...
input
* seq: base sequence
* general_masking_proportion: augmentation proportion
* head_items: araay of head item ids

return:
seq: augmented sequence
'''
def general_masking_tensor(seq, general_masking_proportion, head_items, args):
    tensor_seq=torch.LongTensor(seq).to(args.device)
    head_items_tensor=torch.LongTensor(head_items).to(args.device)

    head_mask=torch.isin(tensor_seq, head_items_tensor).int() 

    probability_of_one = general_masking_proportion 
    probability_tensor = torch.full(size=head_mask.shape, fill_value=probability_of_one)
    tensor2 = torch.bernoulli(probability_tensor).to(args.device)
    mask=torch.mul(head_mask, tensor2.long()) 
