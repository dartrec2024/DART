...
if __name__ == "__main__":
    ...
    head_items, tail_items, train_items, frequency, sorted_items = head_tail_split(user_train, float(1-args.tail_proportion), args.dataset)
    tailset=set(tail_items)
    ...
    if args.inference_only:
        ...
        # do clustering
            ...
            print("do clustering")
            head_clusterid, clusters = clustering(user_train, head_items, tail_items, sorted_items, itemnum, model, args.num_cluster, args)
    ...
    for epoch in tqdm(range(1, args.num_epochs + 1)):
        ...
        for step in tqdm(range(num_batch)):
            ...
            if epoch > args.start_curri:
                # synthetic sequences for contrastive learning 
                masked=general_masking_tensor(seq.copy(), curriculum_function(epoch,args.general_masking_proportion,args.curri_gd, args.start_curri), head_items, args).to(args.device)
                strong_cate = cate_based_item_changing_tensor(seq.copy(), curriculum_function(epoch,args.strong_change_proportion,args.curri_gd, args.start_curri), head_items, clusters, head_clusterid, args).to(args.device)
                weak_cate = cate_based_item_changing_tensor(seq.copy(), curriculum_function(epoch,args.weak_change_proportion,args.curri_gd, args.start_curri), head_items, clusters, head_clusterid, args).to(args.device)             
                
                # synthetic sequences for data augmentation
                if step % 10 == 1: 
                    seq=general_masking_tensor(seq.copy(), curriculum_function(epoch,args.general_masking_proportion,args.curri_gd, args.start_curri), head_items, args).to(args.device)
                elif step % 10 in [5, 8]: 
                    seq = cate_based_item_changing_tensor(seq.copy(), curriculum_function(epoch,args.strong_change_proportion,args.curri_gd, args.start_curri), head_items, clusters, head_clusterid, args).to(args.device)
                else:
                ...
                if step % 10 == 5: 
                    seq=general_masking_tensor(seq.copy(), args.general_masking_proportion, head_items, args).to(args.device)
                elif epoch > 10 and step % 10 in [1, 9]: 
                    seq = cate_based_item_changing_tensor(seq.copy(), args.strong_change_proportion, head_items, clusters, head_clusterid, args).to(args.device)
                else:
                    seq=torch.LongTensor(seq).to(args.device)
